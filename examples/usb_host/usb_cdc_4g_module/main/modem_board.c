...
#define LED_RED_SYSTEM_GPIO                 BOARD_IO_LED_YELLOW
#define LED_BLUE_WIFI_GPIO                  0
#define LED_GREEN_4GMODEM_GPIO              BOARD_IO_LED_GREEN
#define MODEM_POWER_GPIO                    0
#define MODEM_RESET_GPIO                    BOARD_IO_FREE_1
...
static esp_err_t my_recov(esp_modem_recov_resend_t *retry_cmd, esp_err_t err, int timeouts, int errors)
{
    ...
    if (err == ESP_ERR_TIMEOUT) {
        if (timeouts < 2) {
            ...
            if(LED_RED_SYSTEM_GPIO) led_indicator_start(led_system_handle, BLINK_FACTORY_RESET);
            modem_board_t *board = __containerof(dce, modem_board_t, parent);
            board->reset(dce);
                           ...
                           int32_t event_id, void *event_data)
{
    ...
    if (event_base == IP_EVENT) {
        ...
        if (event_id == IP_EVENT_PPP_GOT_IP) {
            ...
            if(LED_GREEN_4GMODEM_GPIO) led_indicator_start(led_4g_handle, BLINK_CONNECTED);
            xEventGroupSetBits(connection_events, CONNECT_BIT);

        } else if (event_id == IP_EVENT_PPP_LOST_IP) {
            ESP_LOGI(TAG, "Modem Disconnect from PPP Server");
            if(LED_GREEN_4GMODEM_GPIO) led_indicator_stop(led_4g_handle, BLINK_CONNECTED);
            if(LED_GREEN_4GMODEM_GPIO) led_indicator_start(led_4g_handle, BLINK_CONNECTING);
        ...
        switch (event_id) {
            case WIFI_EVENT_AP_STACONNECTED:
                if (++active_station_num > 0) {
                    if(LED_BLUE_WIFI_GPIO) led_indicator_start(led_wifi_handle, BLINK_CONNECTED);
                }
                //ESP_LOGI(TAG, "station "MACSTR" join, AID=%d", MAC2STR(event->mac), event->aid);
            break;
            case WIFI_EVENT_AP_STADISCONNECTED:
                if (--active_station_num == 0) {
                    if(LED_BLUE_WIFI_GPIO) led_indicator_stop(led_wifi_handle, BLINK_CONNECTED);
                    if(LED_BLUE_WIFI_GPIO) led_indicator_start(led_wifi_handle, BLINK_CONNECTING);
                }
                //ESP_LOGI(TAG, "station "MACSTR" join, AID=%d", MAC2STR(event->mac), event->aid);
            break;
            default:
...
esp_netif_t *modem_board_init(modem_config_t *config)
{
    ...
    if (ret == ESP_OK) {
        ...
        if(LED_RED_SYSTEM_GPIO) led_indicator_start(led_system_handle, BLINK_CONNECTED);//solid red, internal error
    }
