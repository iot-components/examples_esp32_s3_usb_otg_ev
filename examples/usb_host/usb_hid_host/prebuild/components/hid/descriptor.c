...
static int parse_interface(struct libusb_interface *usb_interface, const uint8_t *buffer, int size)
{
    ...
    while (size >= LIBUSB_DT_INTERFACE_SIZE) {
        ...
        if_desc = (const usb_desc_intf_t *)buffer;
...
{
    const usb_desc_ep_t *ep_desc = (const usb_desc_ep_t *)buff;
...
{
    const usb_desc_intf_t *desc = (const usb_desc_intf_t *)buff;
